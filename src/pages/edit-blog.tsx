...
function EditBlog() {
  ...
  const validateFormData = () => {
    ...
    if (!imageLinkRegex.test(formData.imageLink)) {
      toast.error('Link da imagem deve ser com extensão .jpg, .jpeg, .webp ou .png');
      return false;
    }
    if (formData.categories.length > 3) {
      toast.error('Selecione até 3 categorias.');
      return false;
    }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (validateFormData()) {
      try {
        const response = await axios.patch(process.env.REACT_APP_API_PATH + `/api/posts/${postId}`, formData);

        if (response.status === 200) {
          toast.success('Novo texto criado com sucesso!');
          navigate('/');
        } else {
        ...
        <form onSubmit={handleSubmit} className="md:w-5/6 lg:w-2/3">
          ...
          <div className="mb-2">
            ...
            <input
              type="text"
              name="title"
              placeholder="Informe aqui o título do texto"
              autoComplete="off"
